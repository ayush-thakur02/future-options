...
1. **Bundled packs** — every ``plugin.py`` anywhere under ``niftypulse.plugins``,
...
2. **Third-party distributions** — anything advertising the
   ``niftypulse.plugins`` entry-point group, so a plugin can live in its own
   installed package.
...
BUILTIN_PACKAGE = "niftypulse.plugins"
ENTRY_POINT_GROUP = "niftypulse.plugins"
