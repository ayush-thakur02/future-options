...
def login(
    ...
    from .data.upstox_auth import interactive_login
...
def fetch(
    ...
    settings = _settings()
    source = MarketDataSource(settings, offline=offline)
    try:
        frame = source.load_history(days=days, refresh=True)
    finally:
        source.close()
...
def train(
    ...
    source = MarketDataSource(settings, offline=offline)
    try:
        bars = source.load_history(days=days or 400, refresh=bool(days), quiet=False)
    finally:
        source.close()
...
def backtest(
    ...
    source = MarketDataSource(settings, offline=offline)
    try:
        bars = source.load_history(days=200, refresh=False, quiet=True)
    finally:
        source.close()
    ...
    scores, label = _resolve_scores(strategy, settings, bars, horizon, source_offline=offline)
...
def _resolve_scores(strategy: str, settings, bars, horizon: int, source_offline: bool = False):
...
def dashboard(
    ...
    try:
        if source.access_token is None:
            _run_offline_dashboard(engine, source, speed, refresh, timeframe)
        else:
            _run_live_dashboard(engine, source, refresh, timeframe)
    finally:
        source.close()


def _run_offline_dashboard(engine, source, speed: float, refresh: float, timeframe: int) -> None:
    from .data.synthetic import generate_candles, generate_ticks
    from .ui.dashboard import Dashboard

    console.print("[cyan]offline replay mode[/] — set up Upstox credentials for live data\n")
    bars = generate_candles(days=5, bar_minutes=timeframe, seed=17)
    engine.bootstrap(history=bars)
    ticks = generate_ticks(bars, ticks_per_bar=6)
...
def _run_live_dashboard(engine, source, refresh: float, timeframe: int) -> None:
    from .ui.dashboard import Dashboard

    bars = source.load_history(days=15, refresh=True, quiet=False)
    engine.bootstrap(history=bars)
    dash = Dashboard(symbol=engine.settings.symbol, timeframe=f"{timeframe}m", refresh=refresh)
...
def strategies() -> None:
    ...
    settings = _settings()
    source = MarketDataSource(settings, offline=True)
    try:
        bars = source.load_history(days=20, refresh=False, quiet=True)
    finally:
        source.close()
...
def doctor() -> None:
    ...
    if not token:
        from .data.upstox_auth import TokenStore
