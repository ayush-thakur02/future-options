...
def config_from(params: dict) -> SimulatorConfig:
    ...
    return SimulatorConfig(
        ...
        decision_seconds=float(params.get("decision_seconds", 30.0)),
        ...
        weights=PolicyWeights(
            ...
            ai=float((weights or {}).get("ai", 0.9)),
            model=float((weights or {}).get("model", 0.6)),
            projection=float((weights or {}).get("projection", 1.2)),
            momentum=float((weights or {}).get("momentum", 0.4)),
        ),
    )
